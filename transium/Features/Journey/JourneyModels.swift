...
public nonisolated struct JourneySegment: Codable, Equatable, Sendable, Identifiable {
    ...
    /// The index into `stops` nearest the device's current position — the same live-GPS-as-
    /// bus-proxy approach `liveRemaining` uses for a bus leg's ETA. Nil when this segment
    /// carries no stops at all. Defaults to the boarding stop (index 0) with no live position
    /// yet, since there's no evidence of having moved past it. Backs both `liveStopsRemaining`
    /// and the trip-details timeline's "already passed" styling for individual stops.
    public func liveStopIndex(from currentLocation: CLLocationCoordinate2D?) -> Int? {
    ...
    public func liveStopsRemaining(from currentLocation: CLLocationCoordinate2D?) -> Int? {
        guard let stops, stops.count > 1, let nearestIndex = liveStopIndex(from: currentLocation) else { return nil }
